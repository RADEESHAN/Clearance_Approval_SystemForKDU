...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (!$row_users) {
       $sql_qdmlogin = "SELECT * FROM qdmlogin WHERE username='" . $username . "' AND password='" . $password . "'";
       $result_qdmlogin = mysqli_query($data, $sql_qdmlogin);
       $row_qdmlogin = mysqli_fetch_array($result_qdmlogin);

      if ($row_qdmlogin) {
           $_SESSION["username"] = $username;
           $usertype = $row_qdmlogin["usertype"];
       }
    ...
    if (isset($usertype)) {
        ...
        switch ($usertype) {
            case "student":
                header("location:../students/student3.php");
                break;
            case "ma":
                    header("location:../Ma/ma.php");
                    break;
            ...
            case "admin":
                    header("location:../Admin/admin.php");
                        break;
            case "ab":
                    header("location:../ABofficer/ab.php");
                        break;

            case "adjutant":
                    header("location:../adjutant/adjutant.php");
                        break;
            default:
    ...
    <style>
        ...
        *{
        ...
        body {
            ...
            align-items: center;
            min-height: 100vh;
            background-image: url('kduBG.jpg'); /* Path to your background image */
